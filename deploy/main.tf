...
data "aws_ami" "WebApache" {
  ...
  filter {
    name   = "tag:Name"
    values = ["${var.deploy_env}-${var.project_name}-AMI"]
  }
  most_recent = true
}
...
data "aws_subnet" "subnet-public-1" {
  tags = {
    Name = "${var.deploy_env}-subnet-public-1"
  }
}

data "aws_subnet" "subnet-public-2" {
  tags = {
    Name = "${var.deploy_env}-subnet-public-2"
  }
}

data "aws_subnet" "subnet-public-3" {
  tags = {
    Name = "${var.deploy_env}-subnet-public-3"
  }
}

data "aws_subnet" "subnet-private-1" {
  tags = {
    Name = "${var.deploy_env}-subnet-private-1"
  }
}

data "aws_subnet" "subnet-private-2" {
  tags = {
    Name = "${var.deploy_env}-subnet-private-2"
  }
}

data "aws_subnet" "subnet-private-3" {
  tags = {
    Name = "${var.deploy_env}-subnet-private-3"
  }
}
